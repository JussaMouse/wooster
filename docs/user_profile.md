...
This document outlines a conceptual approach for a "User Profile" system, previously referred to as "UserRAG." It is designed to enable Wooster to learn about the user's preferences, facts, and context from conversations, while distinguishing this from project-specific knowledge handled by the existing ProjectRAG system.
...
To populate User Profile accurately and distinguish its content from ProjectRAG, a "User Knowledge Extractor" module is essential. This would likely be an LLM-powered component that processes each conversational turn.
...
3.  **Storing in User Profile:**
    ...
    *   These new embeddings are added to the dedicated User Profile vector store (`vector_data/user_profile_store/`).
...
*   **Example 1: Discussing a Woody Allen film while the "Romantic Comedy Screenplay" project is open.**
    ...
    *   **User Knowledge Extractor's Analysis & Output for User Profile:**
        *   **Stored:** "User has seen Annie Hall [context: general]", "User loved Annie Hall [context: general]".
        *   **Filtered Out/Ignored:** Specific details about Annie Hall's dialogue style (general film knowledge, not user-specific), Wooster's suggestion for Scene 5 (project-specific creative suggestion). The user's thought process ("thinking about dialogue for this scene") is generally conversational state, not necessarily a persistent fact about the user for User Profile unless frequently repeated as a general interest.
    *   **Outcome:** User Profile learns the user's film preference without being cluttered by the screenplay's content or general film analysis.
...
*   **Example 2: Stating a writing preference while the "Romantic Comedy Screenplay" project is open.**
    ...
    *   **Outcome:** User Profile stores this preference and correctly attributes it to the specific context of writing a romantic comedy screenplay.
...
2.  The query to User Profile could be augmented with the *current project context* (if a project is loaded). For instance: "What are the user's known writing style preferences relevant to the 'Romantic Comedy Screenplay' project, or any general writing preferences?"
3.  User Profile would retrieve facts, prioritizing those that match the current project context. It would also provide general preferences if no project-specific ones are found, or if the query is broad and not tied to a specific project.
...
*   **Managing Ambiguity:** LLMs can misinterpret nuances or context. The system might require mechanisms for the user to review, confirm, or correct what User Profile has "learned" about them.
...
*   **Evolution and Staleness of Preferences:** User preferences and facts can change over time. User Profile would need a strategy for updating, versioning, or allowing the decay of older, potentially outdated information.
...
*   **Privacy and Control:** Users should have transparency and control over the information stored in their User Profile profile.
...
The User Knowledge Extractor, a core part of the User Profile, analyzes conversation snippets (user input and Wooster's responses) to identify and learn facts about you.
...
**Recommendation:**
*   Before enabling or widely using the User Profile feature with a cloud-based LLM for extraction, ensure you understand these privacy implications.
*   Ideally, when implemented, Wooster should provide a clear, one-time notification or require explicit user opt-in before the User Profile system begins learning and storing information when using a cloud-based extractor LLM.

## Current System State Relative to User Profile Implementation

Wooster's current architecture already possesses several components that would be foundational or reusable for implementing a User Profile system:
...
*   **Vector Store Management (`src/memoryVector.ts`, `src/projectIngestor.ts`):**
    ...
    *   Use of `HuggingFaceTransformersEmbeddings` (`Xenova/all-MiniLM-L6-v2`) for local text embedding. This same model can be used for consistency in the User Profile.
    ...
    *   The existing `initVectorStore` could serve as a template for an `initUserProfileStore` function.
...
*   **Modular Structure:** The codebase is already organized into modules (e.g., for memory, agent logic, project ingestion), making it easier to introduce new modules for User Profile and the User Knowledge Extractor.
*   **Configuration (`projects.json`, `.env`):** While not directly for user profiles, the system is accustomed to external configuration, which could be extended if User Profile requires specific settings.

**What's Missing / Needs to be Built:**

*   **User Knowledge Extractor Module:** A new TypeScript module (e.g., `src/userKnowledgeExtractor.ts`) containing the LLM prompting logic to analyze conversations and extract user-specific facts with context. This module forms the heart of the User Profile's learning capability.
*   **User Profile Vector Store Initialization & Management:** Specific logic to create, load, and save the `user_profile_store/` (likely in `src/memoryVector.ts` or a new dedicated file, e.g., `src/userProfileMemory.ts`).
*   **Integration into REPL/Agent Flow (Learning Aspect):** Code in `src/index.ts` (REPL loop) to call the User Knowledge Extractor after each relevant conversational turn and add extracted information to the User Profile store. This is a core system process.
*   **Agent Tool: `recall_user_profile` (Recall Aspect):**
    *   The primary way the agent accesses User Profile is through the dedicated `recall_user_profile` agent tool. This allows the agent (via `AgentExecutor`) to make an intentional decision to query for user-specific information when it deems it relevant.
    *   This tool is defined as a `DynamicTool` within `src/agentExecutorService.ts`.
    *   **For the detailed specification of this tool, including its exact name, the description provided to the agent, input/output schemas, and specific configuration notes, please see its dedicated documentation file: `docs/tools/TOOL_UserProfileRecall.MD`.**
...
*   **(Optional) User Profile Management Plugin:** A separate plugin could be developed to offer user-facing REPL commands for managing their User Profile data (e.g., `list_my_learned_facts`, `forget_user_profile_fact <id>`).

## Simplest Path to Initial User Profile Setup (Proof of Concept for Learning & Tool-Based Recall)

To implement a basic version of User Profile, focusing on the core learning loop and the agent tool for recall:
...
2.  **Modify `src/memoryVector.ts` (or create `src/userProfileMemory.ts`):**
    *   Add a new function `initUserProfileStore(): Promise<FaissStore>`, similar to `initVectorStore`, but using a different path (e.g., `vector_data/user_profile_store/`).
    *   Add a function `addUserFactToProfileStore(fact: string, store: FaissStore): Promise<void>`. This will embed the fact and add it to the User Profile store. Re-use `HuggingFaceTransformersEmbeddings`.

3.  **Integrate into `src/index.ts` (Core Learning Process):**
    *   **Initialization:**
        *   Declare `let userProfileStore: FaissStore;`
        *   In `main()`, call `userProfileStore = await initUserProfileStore();`
    *   **After each conversational turn (in the `rl.on('line', ...)` callback, after Wooster's response is generated and before the next prompt):**
        ...
        *   If `userFact` is not null and not empty:
            *   `await addUserFactToProfileStore(userFact, userProfileStore);`
            *   `await userProfileStore.save('path/to/user_profile_store/');` (Save after each addition for simplicity in PoC).
            *   Optionally, log that a fact was learned: `console.log('[User Profile Learned]:', userFact);`

4.  **Define the `recall_user_profile` Agent Tool (Implemented in `agentExecutorService.ts`):**
    *   The `recall_user_profile` tool is implemented as a `DynamicTool` within `src/agentExecutorService.ts`.
    *   It uses a function like `recallUserProfileFunc` (from `src/tools/userProfileTool.ts`) to perform the similarity search against the User Profile store.
    *   **Refer to `docs/tools/TOOL_UserProfileRecall.MD` for the complete definition, agent-facing description, and usage guidelines for this tool.**
    *   **Testing:** The agent, as part of the `AgentExecutor` framework, should now be able to decide to use this tool based on its description. You'd prompt Wooster with tasks where recalling user preferences would be beneficial and observe if the agent attempts to use `recall_user_profile`.

**This "simplest path" PoC focuses on:**

*   Setting up the User Profile learning cycle.
*   Defining the agent tool for User Profile recall so the agent *can* use it.
...
*   Full integration of User Profile context into the main RAG chain for passive awareness.
*   Highly sophisticated prompt engineering for the User Knowledge Extractor or the main agent to perfectly balance tool use.
*   A UI/Plugin for direct user management of User Profile facts.
...
User Profile can be enabled or disabled globally via settings in your `.env` file, located in the project root. By default, User Profile is **disabled** based on the `DEFAULT_CONFIG` in `src/configLoader.ts` if the specific environment variable is not set.

To control User Profile, set the following environment variable in your `.env` file:

```env
# .env example for User Profile
USER_PROFILE_ENABLED=false # Set to true to enable User Profile
```

If `USER_PROFILE_ENABLED` is set to `false` (or if the variable is omitted and the default is `false`):
- No user knowledge will be extracted or saved by the User Profile system.
- If the `recall_user_profile` tool is invoked, it should ideally inform the agent that User Profile is disabled or simply return no information.
...
```env
# .env example for custom User Profile prompt
USER_PROFILE_EXTRACTOR_LLM_PROMPT="From the user's last message ({userInput}) and my response ({assistantResponse}) in the context of project '{currentProjectName}', identify any explicit statements of preference or personal facts the user has shared. List them clearly."
```

- If `USER_PROFILE_EXTRACTOR_LLM_PROMPT` is not set in your `.env` file, is an empty string, or is omitted, a system default prompt (defined within `src/userKnowledgeExtractor.ts`) will be used.
