...
class NextActionsPluginDefinition implements WoosterPlugin {
  ...
  private async readNextActionsFromFile(): Promise<TaskItem[]> {
    ...
    lines.forEach((line, index) => {
      ...
      if (task) {
        ...
        this.logMsg(LogLevel.WARN, `Failed to parse line ${index + 1} in next_actions.md: "${line}"`);
      }
    });
    return tasks;
  }
  ...
  public async addTask(description: string, contextFromInput?: string | null, projectFromInput?: string | null, dueDate?: string | null): Promise<TaskItem> {
    return this.performFileOperation(async () => {
      ...
      this.logMsg(LogLevel.INFO, `Tasks array now has ${tasks.length} items. Writing...`);
      await this.writeNextActionsToFile(tasks);
      
      const fullPath = this.getFullPath(this.nextActionsFilePath);
      if (!fs.existsSync(fullPath)) {
          throw new Error(`CRITICAL: File ${fullPath} does not exist after write operation!`);
      }
...
`;
  ...
  getAgentTools?(): DynamicTool[] {
    ...
    const addNextActionTool = new DynamicTool({
        name: "addNextAction",
        description: "Adds a new next action. Input can be a simple string (the task description) OR a JSON string with keys: 'description' (required), 'context' (@Tag), 'project' (+Tag), 'dueDate' (YYYY-MM-DD). Example: 'Buy milk' OR '{\"description\": \"Buy milk\", \"context\": \"@errands\"}'",
        func: async (jsonInput: string) => {
        ...
        try {
            ...
            // Handle potential { input: "..." } wrapper often generated by LangChain agents
            if (parsed.input) {
                if (typeof parsed.input === 'string') {
                    try {
                        const inner = JSON.parse(parsed.input);
                        parsed = { ...parsed, ...inner }; // Merge inner
                    } catch {
                        // If input is a string but not JSON, treat it as the description
                        // e.g. { input: "Buy milk" }
                        if (!parsed.description) parsed.description = parsed.input;
                    }
                } else if (typeof parsed.input === 'object') {
                    // Handle nested object input, e.g. { input: { description: "Buy milk" } }
                    parsed = { ...parsed, ...parsed.input };
                }
            }
        } catch {
            // If not JSON, treat the whole string as description
            parsed = { description: jsonInput };
        }

                const description = parsed.description || parsed.title || parsed.text || parsed.task || parsed.body;
