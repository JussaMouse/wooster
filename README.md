...
Wooster has been upgraded with significant performance and capability improvements:

- **3-Tier Intelligent Routing**: Automatically routes queries to the optimal model (Router → Fast → Thinking)
- **Local-First Architecture**: Run entirely on Apple Silicon with [mlx-box](docs/mlx-box.md) (no cloud required)
- **Enhanced Memory System**: Episodic memory, semantic profile, and "where we left off" context
- **New Plugins**: [Habits](docs/plugins/habits.md) tracking, [Goals](docs/plugins/goals.md) with milestones, [Proactive insights](docs/plugins/insights.md)
...
*   **Habit Tracking (`habits` plugin)**: Track daily, weekly, or monthly habits with streaks and statistics. See [Habits Plugin](docs/plugins/habits.md).
*   **Goal Tracking (`goals` plugin)**: Set and track long-term goals with milestones and progress. See [Goals Plugin](docs/plugins/goals.md).
*   **Proactive Insights (`insights` plugin)**: Get weekly insights, habit suggestions, and organization tips based on your patterns. See [Insights Plugin](docs/plugins/insights.md).
...
```bash
pnpm run build      # Fast esbuild build (~48ms)
pnpm run typecheck  # TypeScript type checking (optional, slow)
pnpm run build:tsc  # Traditional tsc build (requires 12GB+ heap)
...
For fully local operation, Wooster supports [mlx-box](docs/mlx-box.md) with intelligent 3-tier routing:
...
See [Performance Upgrade Guide](docs/wooster-performance-upgrade-guide.md) for complete setup instructions.
...
- [Performance Upgrade Guide](docs/wooster-performance-upgrade-guide.md) - Local model optimization
- **Plugin Docs**: [Habits](docs/plugins/habits.md) | [Goals](docs/plugins/goals.md) | [Insights](docs/plugins/insights.md)
